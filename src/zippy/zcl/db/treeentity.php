...
abstract class TreeEntity extends Entity
{
        ...
        public static function generateTree(\ZCL\Tree\Tree $tree, $fname, $rootname = "Root")
        {
                ...
                $itemlist = $class::find(array('order by' => $meta['pathfield'] . "," . $fname));
        ...
        */
        public function getChildren($all = false)
        {
                ...
                if (!$all) {
                        return self::find(new Criteria($meta['parentfield'] . '=' . $this->fields[$meta['keyfield']]));
                } else {
                        return self::find(new Criteria($meta['keyfield'] . ' <> ' . $this->fields[$meta['keyfield']] . ' and ' . $meta['pathfield'] . " like " . $this->qstr('%' . sprintf('%08s', $this->fields[$meta['keyfield']]) . '%')));
                }
        }
